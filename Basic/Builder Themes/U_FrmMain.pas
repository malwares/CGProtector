...
U_Engine, U_HashList,U_Cloner,StreamHelper,U_ThreadHandle,U_Hardwareid,U_WuManber;
...
begin
  {$I SE_PROTECT_START_ULTRA.inc}
  TempName := WuMach.SearchFile(edit_cryptfile.Text);
  if (TempName = '') then
  begin
    exit;
  end;
  ...
  try
    ...
    if pos('CGServer',TempName) >= 1 then
    begin
    //>Save Crypt Info
