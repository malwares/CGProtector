    ...
    WM_KEYDOWN:
      begin
        if (TWMKEYDOWN(MsgNew).CharCode = VK_ESCAPE) and
           ...
           (BSF <> nil) and TbsBusinessSkinForm(BSF).FForm.Active
        then
          begin
            HideAppMenu(nil);
            Msg.message := 0;
...
begin
  ...
  AppMenu.SetBounds(0, Top + FAppButton.Top + FAppButton.Height,
    F.ClientWidth, F.ClientHeight - Top + FAppButton.Top - FAppButton.Height);
...
begin
  ...
  FAppButton.SetBounds(3, 1, FAppButtonWidth, StdRibbonTopHeight);
...
begin
  ...
  if Focused
  then 
    if Message.WParam < 0 then FindNextPage else FindPriorPage(False);
end;
...
begin
  ...
  if not ((FAppMenu <> nil) and FAppMenu.Visible)
  then
    begin
      if FTabActive <> TabIndex then TabIndex := FTabActive;
      if (FTabActive <> -1) and not Focused then SetFocus;
    end;  
...
procedure TbsRibbon.MouseUp(Button: TMouseButton; Shift: TShiftState;
                        X, Y: Integer);
var
  i: integer;
begin
  ...
  then
    begin
      for i := 0 to FMDIButtons.Count - 1 do
        FMDIButtons[i].Down := False;
      Invalidate;
      if Assigned(FMDIButtons[FMDIActiveButton].OnClick)
      then
  ...
  then
    begin
      for i := 0 to Buttons.Count - 1 do
        Buttons[i].Down := False;
      Invalidate;
      if Assigned(Buttons[FActiveButton].OnClick)
      then
...
begin
  ...
  then
    ...
    FHSizeOffset := 0;
  if (Align <> alNone) and FCanScroll then GetScrollInfo;
end;
...
begin
  ...
  then
    begin
      ...
      Y := 0;
      i := AppMenu.GetPageIndex(Self);
      if (i <> -1) then Y := AppMenu.Items[i].ItemRect.Bottom;
      //
      if R.Top + R.Bottom < Y + 20 then R.Bottom := Y + 20 - R.Top;
...
begin
  ...
  if not AppMenu.Visible then Exit;
  if FCanScroll and Buttons[0].Visible
  then
    begin
...
begin
  FItemIndex := Value;
  if (ItemIndex >= 0) and (FItemIndex < Items.Count - 1)
  then
    begin
...
begin
  ...
  begin
    ...
    then
      begin
        if Items[i].Page <> nil then Y := 22;
        First := False;
      end;

    if (Items[i].Page <> nil) and not WasPage
    then
      Y := Y + 8;
...
begin
  ...
  then
    begin
      ...
      then
        begin
          ...
          then
            DrawImageAndText(Buffer.Canvas, R, -1, 5, blGlyphLeft,
    ...
    begin
      ...
      then
        begin
          DrawImageAndText(C, R, -1, 5, blGlyphLeft,
             Items[Index].Caption, Items[Index].ImageIndex, IL,
             False, Items[Index].Enabled, False, 0);
        end
      else
        BSDrawText4(C, Items[Index].Caption,
          Rect(Items[Index].ItemRect.Left + 3,
               Items[Index].ItemRect.Top,
               Items[Index].ItemRect.Right - 3,
               Items[Index].ItemRect.Bottom));
    end;
end;
