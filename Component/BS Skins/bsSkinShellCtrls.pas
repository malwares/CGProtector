...
begin
  ...
  then
    begin
      ...
      if (SortColumn = 0) or
         ((SortColumn <> 0) and (Pos(TimeSeparator, TbsShellFolder(Item1).Details[4]) <> 0))
      then
      case SortColumn of
      0:
        begin
...
begin
  ...
  FDlgFrm := TbsSelDirDlgForm.CreateEx(Application, CtrlSkinData, FShowToolBar, FToolButtonsTransparent, FToolBarImageList, FToolButtonImageIndex);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
...
begin
  ...
  FDlgFrm := TbsOpenDlgForm.CreateEx(Application, FSaveMode,
    CtrlSkinData, FToolButtonsTransparent, FToolBarImageList);
...
begin
  if Assigned(FOnShow) then FOnShow(Self);
  FDlgFrm := TbsOpenSkinDlgForm.CreateEx(Application, CtrlSkinData, FToolButtonsTransparent, FToolBarImageList);
  with FDlgFrm do
  try
    Caption := Self.Title;
    BSF.BorderIcons := [];
