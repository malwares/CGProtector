...
type
  ...
  TParamMod = (pmByVal, pmByRef, pmConst);
...
begin
  while CharInSet(Buff[P + L], ['a'..'z', 'A'..'Z', '_', '0'..'9']) do
    Inc(L);
...
begin
  while CharInSet(Buff[P + L], ['0'..'9']) do
    Inc(L);
end;

procedure THeaderParser.ScanHexDigits;
begin
  while CharInSet(Buff[P + L], ['0'..'9', 'a'..'f', 'A'..'F']) do
    Inc(L);
end;
...
begin
  ...
  begin
    repeat
      ...
      if (NP = 0) and StrEql('Self', CurrToken) then
        IsFakeMethod := true;
...
begin
  ...
  if CurrToken = '+' then
    ...
    Op := OP_MINUS
  else if CurrToken = 'or' then
    Op := OP_OR
  else if CurrToken = 'xor' then
    Op := OP_XOR
  else
    Op := 0;
  ...
  begin
    ...
    if VarType(result) = varString then
      ...
      else if TBaseSymbolTable(symbol_table)[LastFactorTypeId].FinalTypeId in IntegerTypes then
      begin
        W := 0;
...
{$ENDIF}
  ...
  begin
    ...
    i := Pos('.', CurrToken);
    if i > 0 then
      CurrToken[i] := DecimalSeparator;

    if LastFactorTypeId = typeCURRENCY then
      result := StrToCurr(CurrToken)
    else
      result := StrToFloat(CurrToken);
    Call_SCANNER;
  end
