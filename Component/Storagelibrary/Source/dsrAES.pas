...
{                                                       }
{      Rijndael block chipher implementation by         }
{                     DeepSoftware                      }
{                                                       }
{                  version 0.14 (014)                   }
{               build 014  (19 May 2004)                }
{                                                       }
...
{$DEFINE DSRAES_TEST}
{$IFDEF VER100}
  {$DEFINE DSRAES_INTMATRIX}
{$ENDIF}

{$IFDEF VER100}  {$DEFINE DSRAES_PINT}  {$ENDIF}
{$IFDEF VER110}  {$DEFINE DSRAES_PINT}  {$ENDIF}
{$IFDEF VER120}  {$DEFINE DSRAES_PINT}  {$ENDIF}
{$IFDEF VER125}  {$DEFINE DSRAES_PINT}  {$ENDIF}
{$IFDEF VER130}  {$DEFINE DSRAES_PINT}  {$ENDIF}
...
//errors
  AESCIPHER_NO_DATA = -1;
  AESCIPHER_NO_DEST = -2;
  AESCIPHER_NEED_MORE_DEST_SIZE = -3;
  AESCIPHER_BAD_DATASIZE = -4;
  AESCIPHER_BAD_DATA = -5;
...
{$ENDIF}
  ...
  TBlockWord8 = array[0..15] of Word8;
  PBlockWord8 = ^TBlockWord8;

  TArrayWord8 = array[0..31] of Word8;
  PArrayWord8 = ^TArrayWord8;

  TArrayWord32 = array [0..4*(MAXROUNDS+1)-1] of Word32;
  PArrayWord32 = ^TArrayWord32;

  TAESChipherMode = (cmECB, cmCBC);
  TAESChipherKeyBits = (ck128, ck192, ck256);

  TAESChipher = class
  private
    FKey: String;
    FKeyBits: TAESChipherKeyBits;
    FMode: TAESChipherMode;
    Nr: integer;
    FEncryptRK: TArrayWord32;
    FDecryptRK: TArrayWord32;
    procedure SetKey(const Value: String);
    procedure SetKeyBits(const Value: TAESChipherKeyBits);
  protected
    KeyNeedUpdate: boolean;
    function GetKeyBitsAsInteger: integer;
    procedure UpdateKeyData(Forced: boolean);
  public
    InitVector: TBlockWord8;

    constructor Create;
    property Mode: TAESChipherMode read FMode write FMode default cmECB;
    property KeyBits: TAESChipherKeyBits read FKeyBits write SetKeyBits default ck128;
    property Key: String read FKey write SetKey;

    function BlocksDecrypt(Src: Pointer; Count: integer; Dest: Pointer): integer; //count in bytes
    function BlocksEncrypt(Src: Pointer; Count: integer; Dest: Pointer): integer; //count in bytes
{$IFDEF DSRAES_TEST}
    procedure BlockDecryptRounds(block: PBlockWord8; rounds: integer);
    procedure BlockEncryptRounds(block: PBlockWord8; rounds: integer);
{$ENDIF}
    {Calc destination buffer size before using below methods:
      run methods with Dest=nil for calc DestCount.
      result = count of processed bytes or Error}
    function BufferDecrypt(Src: Pointer; SrcCount: integer; Dest: Pointer; var DestCount: integer): integer; //count in bytes
    function BufferEncrypt(Src: Pointer; SrcCount: integer; Dest: Pointer; var DestCount: integer): integer; //count in bytes

    {Work with Src from current position
     result = count of processed bytes or Error}
    function StreamDecrypt(Src: TStream; Dest: TStream): integer; //count in bytes
    function StreamEncrypt(Src: TStream; Dest: TStream): integer; //count in bytes

    function StringDecrypt(const Value: string): string;
    function StringEncrypt(const Value: string): string;
  end;
...
const
  Te0: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
  ...
  Te1: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
  ...
  Te2: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Te3: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Te4: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Td0: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Td1: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Td2: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Td3: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Td4: array [0..255] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
 ...
 Rcon: array [0..9] of {$IFDEF DSRAES_INTMATRIX}Integer{$ELSE}Word32{$ENDIF} = (
  $01000000, $02000000, $04000000, $08000000, $10000000,
  $20000000, $40000000, $80000000, $1B000000, $36000000);


type
  B32 = array[1..4] of Word8; // for bytes access in Word32 value. Word32 = B4B3B2B1


function SwapU32(Value: Word32): Word32;
asm
  bswap EAX
end;
...
function RijndaelKeySetupEnc(rk: PArrayWord32; cipherKey: PArrayWord8; keyBits: integer): integer;
var
  temp: Word32;
  i: integer;
begin
  rk^[0] := SwapU32(PWord32(@cipherKey^[ 0])^);
  rk^[1] := SwapU32(PWord32(@cipherKey^[ 4])^);
  rk^[2] := SwapU32(PWord32(@cipherKey^[ 8])^);
  rk^[3] := SwapU32(PWord32(@cipherKey^[12])^);

  if keyBits = 128 then begin
    for i:=0 to 9 do begin
      temp := rk^[3];
      rk^[4] := rk^[0] xor
                (Te4[B32(temp)[3]] and $ff000000) xor
                (Te4[B32(temp)[2]] and $00ff0000) xor
                (Te4[B32(temp)[1]] and $0000ff00) xor
                (Te4[B32(temp)[4]] and $000000ff) xor
                Rcon[i];
      rk^[5] := rk^[1] xor rk^[4];
      rk^[6] := rk^[2] xor rk^[5];
      rk^[7] := rk^[3] xor rk^[6];
  ...
  rk^[4] := SwapU32(PWord32(@cipherKey^[16])^);
  rk^[5] := SwapU32(PWord32(@cipherKey^[20])^);
  ...
  rk^[6] := SwapU32(PWord32(@cipherKey^[24])^);
  rk^[7] := SwapU32(PWord32(@cipherKey^[28])^);
  if keyBits = 256 then begin
    for i:=0 to 6 do begin
      ...
      if i<6 then begin
        temp := rk^[11];
        rk^[12] := rk^[4] xor
                  (Te4[B32(temp)[4]] and $ff000000) xor
                  (Te4[B32(temp)[3]] and $00ff0000) xor
                  (Te4[B32(temp)[2]] and $0000ff00) xor
                  (Te4[B32(temp)[1]] and $000000ff);
...
function RijndaelKeySetupDec(rk: PArrayWord32; cipherKey: PArrayWord8; keyBits: integer): integer;
var
  i, j: integer;
  temp: Word32;
begin
  ...
  //result=Nr
  Result := RijndaelKeySetupEnc(rk, cipherKey, keyBits);
 ...
 {apply the inverse MixColumn transform to all round keys but the first and the last:}
  for i:=1 to Result-1 do begin
    Inc(PWord32(rk), 4);
    rk^[0] := Td0[Te4[(rk^[0] shr 24)        ] and $ff] xor
              Td1[Te4[(rk^[0] shr 16) and $ff] and $ff] xor
              Td2[Te4[(rk^[0] shr  8) and $ff] and $ff] xor
              Td3[Te4[(rk^[0]       ) and $ff] and $ff];
    rk^[1] := Td0[Te4[(rk^[1] shr 24)        ] and $ff] xor
              Td1[Te4[(rk^[1] shr 16) and $ff] and $ff] xor
              Td2[Te4[(rk^[1] shr  8) and $ff] and $ff] xor
              Td3[Te4[(rk^[1]       ) and $ff] and $ff];
    rk^[2] := Td0[Te4[(rk^[2] shr 24)        ] and $ff] xor
              Td1[Te4[(rk^[2] shr 16) and $ff] and $ff] xor
              Td2[Te4[(rk^[2] shr  8) and $ff] and $ff] xor
              Td3[Te4[(rk^[2]       ) and $ff] and $ff];
    rk^[3] := Td0[Te4[(rk^[3] shr 24)        ] and $ff] xor
              Td1[Te4[(rk^[3] shr 16) and $ff] and $ff] xor
              Td2[Te4[(rk^[3] shr  8) and $ff] and $ff] xor
              Td3[Te4[(rk^[3]       ) and $ff] and $ff];
  end;
end;

procedure RijndaelEncrypt(rk: PArrayWord32; Nr: integer; pt, ct: PBlockWord8);
var
  s0, s1, s2, s3, t0, t1, t2, t3: Word32;
begin
  s0 := SwapU32(PWord32(@pt^[ 0])^) xor rk^[0];
  s1 := SwapU32(PWord32(@pt^[ 4])^) xor rk^[1];
  s2 := SwapU32(PWord32(@pt^[ 8])^) xor rk^[2];
  s3 := SwapU32(PWord32(@pt^[12])^) xor rk^[3];

  { round 1: }
  t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[ 4];
  t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[ 5];
  t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[ 6];
  t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[ 7];
  { round 2: }
  s0 := Te0[B32(t0)[4]] xor Te1[B32(t1)[3]] xor Te2[B32(t2)[2]] xor Te3[B32(t3)[1]] xor rk^[ 8];
  s1 := Te0[B32(t1)[4]] xor Te1[B32(t2)[3]] xor Te2[B32(t3)[2]] xor Te3[B32(t0)[1]] xor rk^[ 9];
  s2 := Te0[B32(t2)[4]] xor Te1[B32(t3)[3]] xor Te2[B32(t0)[2]] xor Te3[B32(t1)[1]] xor rk^[10];
  s3 := Te0[B32(t3)[4]] xor Te1[B32(t0)[3]] xor Te2[B32(t1)[2]] xor Te3[B32(t2)[1]] xor rk^[11];
  { round 3: }
  t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[12];
  t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[13];
  t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[14];
  t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[15];
  { round 4: }
  s0 := Te0[B32(t0)[4]] xor Te1[B32(t1)[3]] xor Te2[B32(t2)[2]] xor Te3[B32(t3)[1]] xor rk^[16];
  s1 := Te0[B32(t1)[4]] xor Te1[B32(t2)[3]] xor Te2[B32(t3)[2]] xor Te3[B32(t0)[1]] xor rk^[17];
  s2 := Te0[B32(t2)[4]] xor Te1[B32(t3)[3]] xor Te2[B32(t0)[2]] xor Te3[B32(t1)[1]] xor rk^[18];
  s3 := Te0[B32(t3)[4]] xor Te1[B32(t0)[3]] xor Te2[B32(t1)[2]] xor Te3[B32(t2)[1]] xor rk^[19];
  { round 5: }
  t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[20];
  t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[21];
  t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[22];
  t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[23];
  { round 6: }
  s0 := Te0[B32(t0)[4]] xor Te1[B32(t1)[3]] xor Te2[B32(t2)[2]] xor Te3[B32(t3)[1]] xor rk^[24];
  s1 := Te0[B32(t1)[4]] xor Te1[B32(t2)[3]] xor Te2[B32(t3)[2]] xor Te3[B32(t0)[1]] xor rk^[25];
  s2 := Te0[B32(t2)[4]] xor Te1[B32(t3)[3]] xor Te2[B32(t0)[2]] xor Te3[B32(t1)[1]] xor rk^[26];
  s3 := Te0[B32(t3)[4]] xor Te1[B32(t0)[3]] xor Te2[B32(t1)[2]] xor Te3[B32(t2)[1]] xor rk^[27];
  { round 7: }
  t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[28];
  t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[29];
  t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[30];
  t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[31];
  { round 8: }
  s0 := Te0[B32(t0)[4]] xor Te1[B32(t1)[3]] xor Te2[B32(t2)[2]] xor Te3[B32(t3)[1]] xor rk^[32];
  s1 := Te0[B32(t1)[4]] xor Te1[B32(t2)[3]] xor Te2[B32(t3)[2]] xor Te3[B32(t0)[1]] xor rk^[33];
  s2 := Te0[B32(t2)[4]] xor Te1[B32(t3)[3]] xor Te2[B32(t0)[2]] xor Te3[B32(t1)[1]] xor rk^[34];
  s3 := Te0[B32(t3)[4]] xor Te1[B32(t0)[3]] xor Te2[B32(t1)[2]] xor Te3[B32(t2)[1]] xor rk^[35];
  { round 9: }
  t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[36];
  t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[37];
  t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[38];
  t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[39];
  if Nr > 10 then begin
    { round 10: }
    s0 := Te0[B32(t0)[4]] xor Te1[B32(t1)[3]] xor Te2[B32(t2)[2]] xor Te3[B32(t3)[1]] xor rk^[40];
    s1 := Te0[B32(t1)[4]] xor Te1[B32(t2)[3]] xor Te2[B32(t3)[2]] xor Te3[B32(t0)[1]] xor rk^[41];
    s2 := Te0[B32(t2)[4]] xor Te1[B32(t3)[3]] xor Te2[B32(t0)[2]] xor Te3[B32(t1)[1]] xor rk^[42];
    s3 := Te0[B32(t3)[4]] xor Te1[B32(t0)[3]] xor Te2[B32(t1)[2]] xor Te3[B32(t2)[1]] xor rk^[43];
    { round 11: }
    t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[44];
    t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[45];
    t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[46];
    t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[47];
    if Nr > 12 then begin
      { round 12: }
      s0 := Te0[B32(t0)[4]] xor Te1[B32(t1)[3]] xor Te2[B32(t2)[2]] xor Te3[B32(t3)[1]] xor rk^[48];
      s1 := Te0[B32(t1)[4]] xor Te1[B32(t2)[3]] xor Te2[B32(t3)[2]] xor Te3[B32(t0)[1]] xor rk^[49];
      s2 := Te0[B32(t2)[4]] xor Te1[B32(t3)[3]] xor Te2[B32(t0)[2]] xor Te3[B32(t1)[1]] xor rk^[50];
      s3 := Te0[B32(t3)[4]] xor Te1[B32(t0)[3]] xor Te2[B32(t1)[2]] xor Te3[B32(t2)[1]] xor rk^[51];
      { round 13: }
      t0 := Te0[B32(s0)[4]] xor Te1[B32(s1)[3]] xor Te2[B32(s2)[2]] xor Te3[B32(s3)[1]] xor rk^[52];
      t1 := Te0[B32(s1)[4]] xor Te1[B32(s2)[3]] xor Te2[B32(s3)[2]] xor Te3[B32(s0)[1]] xor rk^[53];
      t2 := Te0[B32(s2)[4]] xor Te1[B32(s3)[3]] xor Te2[B32(s0)[2]] xor Te3[B32(s1)[1]] xor rk^[54];
      t3 := Te0[B32(s3)[4]] xor Te1[B32(s0)[3]] xor Te2[B32(s1)[2]] xor Te3[B32(s2)[1]] xor rk^[55];
    end;
  end;

  Inc(PWord32(rk), Nr shl 2);
  {apply last round and map cipher state to byte array block:}
  s0 := (Te4[B32(t0)[4]] and $ff000000) xor
        (Te4[B32(t1)[3]] and $00ff0000) xor
        (Te4[B32(t2)[2]] and $0000ff00) xor
        (Te4[B32(t3)[1]] and $000000ff) xor
        rk^[0];
  s1 := (Te4[B32(t1)[4]] and $ff000000) xor
        (Te4[B32(t2)[3]] and $00ff0000) xor
        (Te4[B32(t3)[2]] and $0000ff00) xor
        (Te4[B32(t0)[1]] and $000000ff) xor
        rk^[1];
  s2 := (Te4[B32(t2)[4]] and $ff000000) xor
        (Te4[B32(t3)[3]] and $00ff0000) xor
        (Te4[B32(t0)[2]] and $0000ff00) xor
        (Te4[B32(t1)[1]] and $000000ff) xor
        rk^[2];
  s3 := (Te4[B32(t3)[4]] and $ff000000) xor
        (Te4[B32(t0)[3]] and $00ff0000) xor
        (Te4[B32(t1)[2]] and $0000ff00) xor
        (Te4[B32(t2)[1]] and $000000ff) xor
        rk^[3];
  PWord32(@ct^[ 0])^ := SwapU32(s0);
  PWord32(@ct^[ 4])^ := SwapU32(s1);
  PWord32(@ct^[ 8])^ := SwapU32(s2);
  PWord32(@ct^[12])^ := SwapU32(s3);
end;

procedure RijndaelDecrypt(rk: PArrayWord32; Nr: integer; ct, pt: PBlockWord8);
var
  s0, s1, s2, s3, t0, t1, t2, t3: Word32;
begin
  {map byte array block to cipher state and add initial round key:}
  s0 := SwapU32(PWord32(@ct^[ 0])^) xor rk^[0];
  s1 := SwapU32(PWord32(@ct^[ 4])^) xor rk^[1];
  s2 := SwapU32(PWord32(@ct^[ 8])^) xor rk^[2];
  s3 := SwapU32(PWord32(@ct^[12])^) xor rk^[3];
  { round 1: }
  t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[ 4];
  t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[ 5];
  t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[ 6];
  t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[ 7];
  { round 2: }
  s0 := Td0[B32(t0)[4]] xor Td1[B32(t3)[3]] xor Td2[B32(t2)[2]] xor Td3[B32(t1)[1]] xor rk^[ 8];
  s1 := Td0[B32(t1)[4]] xor Td1[B32(t0)[3]] xor Td2[B32(t3)[2]] xor Td3[B32(t2)[1]] xor rk^[ 9];
  s2 := Td0[B32(t2)[4]] xor Td1[B32(t1)[3]] xor Td2[B32(t0)[2]] xor Td3[B32(t3)[1]] xor rk^[10];
  s3 := Td0[B32(t3)[4]] xor Td1[B32(t2)[3]] xor Td2[B32(t1)[2]] xor Td3[B32(t0)[1]] xor rk^[11];
  { round 3: }
  t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[12];
  t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[13];
  t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[14];
  t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[15];
  { round 4: }
  s0 := Td0[B32(t0)[4]] xor Td1[B32(t3)[3]] xor Td2[B32(t2)[2]] xor Td3[B32(t1)[1]] xor rk^[16];
  s1 := Td0[B32(t1)[4]] xor Td1[B32(t0)[3]] xor Td2[B32(t3)[2]] xor Td3[B32(t2)[1]] xor rk^[17];
  s2 := Td0[B32(t2)[4]] xor Td1[B32(t1)[3]] xor Td2[B32(t0)[2]] xor Td3[B32(t3)[1]] xor rk^[18];
  s3 := Td0[B32(t3)[4]] xor Td1[B32(t2)[3]] xor Td2[B32(t1)[2]] xor Td3[B32(t0)[1]] xor rk^[19];
  { round 5: }
  t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[20];
  t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[21];
  t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[22];
  t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[23];
  { round 6: }
  s0 := Td0[B32(t0)[4]] xor Td1[B32(t3)[3]] xor Td2[B32(t2)[2]] xor Td3[B32(t1)[1]] xor rk^[24];
  s1 := Td0[B32(t1)[4]] xor Td1[B32(t0)[3]] xor Td2[B32(t3)[2]] xor Td3[B32(t2)[1]] xor rk^[25];
  s2 := Td0[B32(t2)[4]] xor Td1[B32(t1)[3]] xor Td2[B32(t0)[2]] xor Td3[B32(t3)[1]] xor rk^[26];
  s3 := Td0[B32(t3)[4]] xor Td1[B32(t2)[3]] xor Td2[B32(t1)[2]] xor Td3[B32(t0)[1]] xor rk^[27];
  { round 7: }
  t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[28];
  t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[29];
  t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[30];
  t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[31];
  { round 8: }
  s0 := Td0[B32(t0)[4]] xor Td1[B32(t3)[3]] xor Td2[B32(t2)[2]] xor Td3[B32(t1)[1]] xor rk^[32];
  s1 := Td0[B32(t1)[4]] xor Td1[B32(t0)[3]] xor Td2[B32(t3)[2]] xor Td3[B32(t2)[1]] xor rk^[33];
  s2 := Td0[B32(t2)[4]] xor Td1[B32(t1)[3]] xor Td2[B32(t0)[2]] xor Td3[B32(t3)[1]] xor rk^[34];
  s3 := Td0[B32(t3)[4]] xor Td1[B32(t2)[3]] xor Td2[B32(t1)[2]] xor Td3[B32(t0)[1]] xor rk^[35];
  { round 9: }
  t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[36];
  t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[37];
  t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[38];
  t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[39];
  if Nr > 10 then begin
    { round 10: }
    s0 := Td0[B32(t0)[4]] xor Td1[B32(t3)[3]] xor Td2[B32(t2)[2]] xor Td3[B32(t1)[1]] xor rk^[40];
    s1 := Td0[B32(t1)[4]] xor Td1[B32(t0)[3]] xor Td2[B32(t3)[2]] xor Td3[B32(t2)[1]] xor rk^[41];
    s2 := Td0[B32(t2)[4]] xor Td1[B32(t1)[3]] xor Td2[B32(t0)[2]] xor Td3[B32(t3)[1]] xor rk^[42];
    s3 := Td0[B32(t3)[4]] xor Td1[B32(t2)[3]] xor Td2[B32(t1)[2]] xor Td3[B32(t0)[1]] xor rk^[43];
    { round 11: }
    t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[44];
    t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[45];
    t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[46];
    t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[47];
    if Nr > 12 then begin
      { round 12: }
      s0 := Td0[B32(t0)[4]] xor Td1[B32(t3)[3]] xor Td2[B32(t2)[2]] xor Td3[B32(t1)[1]] xor rk^[48];
      s1 := Td0[B32(t1)[4]] xor Td1[B32(t0)[3]] xor Td2[B32(t3)[2]] xor Td3[B32(t2)[1]] xor rk^[49];
      s2 := Td0[B32(t2)[4]] xor Td1[B32(t1)[3]] xor Td2[B32(t0)[2]] xor Td3[B32(t3)[1]] xor rk^[50];
      s3 := Td0[B32(t3)[4]] xor Td1[B32(t2)[3]] xor Td2[B32(t1)[2]] xor Td3[B32(t0)[1]] xor rk^[51];
      { round 13: }
      t0 := Td0[B32(s0)[4]] xor Td1[B32(s3)[3]] xor Td2[B32(s2)[2]] xor Td3[B32(s1)[1]] xor rk^[52];
      t1 := Td0[B32(s1)[4]] xor Td1[B32(s0)[3]] xor Td2[B32(s3)[2]] xor Td3[B32(s2)[1]] xor rk^[53];
      t2 := Td0[B32(s2)[4]] xor Td1[B32(s1)[3]] xor Td2[B32(s0)[2]] xor Td3[B32(s3)[1]] xor rk^[54];
      t3 := Td0[B32(s3)[4]] xor Td1[B32(s2)[3]] xor Td2[B32(s1)[2]] xor Td3[B32(s0)[1]] xor rk^[55];
    end;
  end;
  Inc(PWord32(rk), Nr shl 2);

  {apply last round and map cipher state to byte array block:}
  s0 := (Td4[B32(t0)[4]] and $ff000000) xor
        (Td4[B32(t3)[3]] and $00ff0000) xor
        (Td4[B32(t2)[2]] and $0000ff00) xor
        (Td4[B32(t1)[1]] and $000000ff) xor
        rk^[0];
  s1 := (Td4[B32(t1)[4]] and $ff000000) xor
        (Td4[B32(t0)[3]] and $00ff0000) xor
        (Td4[B32(t3)[2]] and $0000ff00) xor
        (Td4[B32(t2)[1]] and $000000ff) xor
        rk^[1];
  s2 := (Td4[B32(t2)[4]] and $ff000000) xor
        (Td4[B32(t1)[3]] and $00ff0000) xor
        (Td4[B32(t0)[2]] and $0000ff00) xor
        (Td4[B32(t3)[1]] and $000000ff) xor
        rk^[2];
  s3 := (Td4[B32(t3)[4]] and $ff000000) xor
        (Td4[B32(t2)[3]] and $00ff0000) xor
        (Td4[B32(t1)[2]] and $0000ff00) xor
        (Td4[B32(t0)[1]] and $000000ff) xor
        rk[3];
  PWord32(@pt^[ 0])^ := SwapU32(s0);
  PWord32(@pt^[ 4])^ := SwapU32(s1);
  PWord32(@pt^[ 8])^ := SwapU32(s2);
  PWord32(@pt^[12])^ := SwapU32(s3);
end;


{$IFDEF DSRAES_TEST}
procedure RijndaelEncryptRound(rk: PArrayWord32; Nr: integer; block: PBlockWord8; rounds: integer);
var
  i,r: integer;
  s0, s1, s2, s3, t0, t1, t2, t3: Word32;
begin
  {map byte array block to cipher state and add initial round key:}
  s0 := SwapU32(PWord32(@block^[ 0])^) xor rk^[0];
  s1 := SwapU32(PWord32(@block^[ 4])^) xor rk^[1];
  s2 := SwapU32(PWord32(@block^[ 8])^) xor rk^[2];
  s3 := SwapU32(PWord32(@block^[12])^) xor rk^[3];
  Inc(PWord32(rk), 4);
  {Nr - 1 full rounds:}
  if rounds < Nr
    then i := rounds
  ...
  for r:= 0 to i-1 do begin//ray ?? or no -1 ???
    t0 := Te0[s0 shr 24] xor Te1[(s1 shr 16) and $ff] xor Te2[(s2 shr  8) and $ff] xor Te3[s3 and $ff] xor rk^[0];
    t1 := Te0[s1 shr 24] xor Te1[(s2 shr 16) and $ff] xor Te2[(s3 shr  8) and $ff] xor Te3[s0 and $ff] xor rk^[1];
    t2 := Te0[s2 shr 24] xor Te1[(s3 shr 16) and $ff] xor Te2[(s0 shr  8) and $ff] xor Te3[s1 and $ff] xor rk^[2];
    t3 := Te0[s3 shr 24] xor Te1[(s0 shr 16) and $ff] xor Te2[(s1 shr  8) and $ff] xor Te3[s2 and $ff] xor rk^[3];
  ...
  {apply last round and map cipher state to byte array block:}
  if rounds = Nr then begin
    t0 := (Te4[(s0 shr 24)        ] and $ff000000) xor
          (Te4[(s1 shr 16) and $ff] and $00ff0000) xor
          (Te4[(s2 shr  8) and $ff] and $0000ff00) xor
          (Te4[(s3       ) and $ff] and $000000ff) xor
          rk^[0];
    t1 := (Te4[(s1 shr 24)        ] and $ff000000) xor
          (Te4[(s2 shr 16) and $ff] and $00ff0000) xor
          (Te4[(s3 shr  8) and $ff] and $0000ff00) xor
          (Te4[(s0       ) and $ff] and $000000ff) xor
          rk^[1];
    t2 := (Te4[(s2 shr 24)        ] and $ff000000) xor
          (Te4[(s3 shr 16) and $ff] and $00ff0000) xor
          (Te4[(s0 shr  8) and $ff] and $0000ff00) xor
          (Te4[(s1       ) and $ff] and $000000ff) xor
          rk^[2];
    t3 := (Te4[(s3 shr 24)        ] and $ff000000) xor
          (Te4[(s0 shr 16) and $ff] and $00ff0000) xor
          (Te4[(s1 shr  8) and $ff] and $0000ff00) xor
          (Te4[(s2       ) and $ff] and $000000ff) xor
          rk^[3];
    s0 := t0;
    s1 := t1;
    s2 := t2;
  ...
  PWord32(@block^[ 0])^ := SwapU32(s0);
  PWord32(@block^[ 4])^ := SwapU32(s1);
  PWord32(@block^[ 8])^ := SwapU32(s2);
  PWord32(@block^[12])^ := SwapU32(s3);
end;
{$ENDIF}

{$IFDEF DSRAES_TEST}
procedure RijndaelDecryptRound(rk: PArrayWord32; Nr: integer; block: PBlockWord8; rounds: integer);
var
  r,i: integer;
  s0, s1, s2, s3, t0, t1, t2, t3: Word32;
begin
  ...
  for r:= 0 to i-2 do begin//ray ?? or no -1 ???
    t0 := Td0[s0 shr 24] xor Td1[(s3 shr 16) and $ff] xor Td2[(s2 shr  8) and $ff] xor Td3[s1 and $ff] xor rk^[0];
    t1 := Td0[s1 shr 24] xor Td1[(s0 shr 16) and $ff] xor Td2[(s3 shr  8) and $ff] xor Td3[s2 and $ff] xor rk^[1];
    t2 := Td0[s2 shr 24] xor Td1[(s1 shr 16) and $ff] xor Td2[(s0 shr  8) and $ff] xor Td3[s3 and $ff] xor rk^[2];
    t3 := Td0[s3 shr 24] xor Td1[(s2 shr 16) and $ff] xor Td2[(s1 shr  8) and $ff] xor Td3[s0 and $ff] xor rk^[3];
  ...
  {complete the last round and map cipher state to byte array block:}
  t0 := (Td4[(s0 shr 24)        ] and $ff000000) xor
        (Td4[(s3 shr 16) and $ff] and $00ff0000) xor
        (Td4[(s2 shr  8) and $ff] and $0000ff00) xor
        (Td4[(s1       ) and $ff] and $000000ff);
  t1 := (Td4[(s1 shr 24)        ] and $ff000000) xor
        (Td4[(s0 shr 16) and $ff] and $00ff0000) xor
        (Td4[(s3 shr  8) and $ff] and $0000ff00) xor
        (Td4[(s2       ) and $ff] and $000000ff);
  t2 := (Td4[(s2 shr 24)        ] and $ff000000) xor
        (Td4[(s1 shr 16) and $ff] and $00ff0000) xor
        (Td4[(s0 shr  8) and $ff] and $0000ff00) xor
        (Td4[(s3       ) and $ff] and $000000ff);
  t3 := (Td4[(s3 shr 24)        ] and $ff000000) xor
        (Td4[(s2 shr 16) and $ff] and $00ff0000) xor
        (Td4[(s1 shr  8) and $ff] and $0000ff00) xor
        (Td4[(s0       ) and $ff] and $000000ff);
  if rounds=Nr then begin
    t0 := rk^[0] xor t0;
    t1 := rk^[1] xor t1;
    t2 := rk^[2] xor t2;
  ...
  PWord32(@block^[ 0])^ := SwapU32(t0);
  PWord32(@block^[ 4])^ := SwapU32(t1);
  PWord32(@block^[ 8])^ := SwapU32(t2);
  PWord32(@block^[12])^ := SwapU32(t3);
end;
{$ENDIF}

{ TAESChipher }

function TAESChipher.BlocksDecrypt(Src: Pointer; Count: integer; Dest: Pointer): integer;
var
  i, NumBlocks: integer;
  PInitVector: PBlockWord8;
begin
  Result := 0;
  if Dest=nil then begin
    Result := AESCIPHER_NO_DEST;
    Exit;
  end;
  if Count mod SizeOf(TBlockWord8) <>0 then begin
    Result := AESCIPHER_BAD_DATASIZE;
    Exit;
  end;

  UpdateKeyData(False);
  NumBlocks := Count div SizeOf(TBlockWord8);
  case Mode of
    cmECB:
          begin
            for i:=1 to NumBlocks do begin
              RijndaelDecrypt(@FDecryptRK, Nr, Src, Dest);
              Inc(PWord8(Src),  SizeOf(TBlockWord8));
              Inc(PWord8(Dest), SizeOf(TBlockWord8));
            end;
            Result := Count;
          end;
    cmCBC:
          begin
            ...
            for i:=1 to NumBlocks do begin
              RijndaelDecrypt(@FDecryptRK, Nr, Src, Dest);
              PWord32(Integer(Dest)+ 0)^ := PWord32(Integer(Dest)+ 0)^ xor PWord32(@(PInitVector^[ 0]))^;
              PWord32(Integer(Dest)+ 4)^ := PWord32(Integer(Dest)+ 4)^ xor PWord32(@(PInitVector^[ 4]))^;
              PWord32(Integer(Dest)+ 8)^ := PWord32(Integer(Dest)+ 8)^ xor PWord32(@(PInitVector^[ 8]))^;
              PWord32(Integer(Dest)+12)^ := PWord32(Integer(Dest)+12)^ xor PWord32(@(PInitVector^[12]))^;
              PInitVector := Src;
...
function TAESChipher.BlocksEncrypt(Src: Pointer; Count: integer; Dest: Pointer): integer;
var
  i, NumBlocks: integer;
  PInitVector: PBlockWord8;
  block: TBlockWord8;
begin
  ...
  UpdateKeyData(False);
  NumBlocks := Count div SizeOf(TBlockWord8);//(128{GetKeyBitsAsInteger} div 8);
  case Mode of
    cmECB:
          begin
            for i:=1 to NumBlocks do begin
              RijndaelEncrypt(@FEncryptRK, Nr, Src, Dest);
              Inc(PWord8(Src),  SizeOf(TBlockWord8));
              Inc(PWord8(Dest), SizeOf(TBlockWord8));
            end;
            Result := Count;
    ...
    cmCBC:
          begin
            PInitVector := @InitVector;
            for i:=1 to NumBlocks do begin
              PWord32(@block[ 0])^ := PWord32(@(PInitVector^[ 0]))^ xor PWord32(Integer(Src)+ 0)^;
              PWord32(@block[ 4])^ := PWord32(@(PInitVector^[ 4]))^ xor PWord32(Integer(Src)+ 4)^;
              PWord32(@block[ 8])^ := PWord32(@(PInitVector^[ 8]))^ xor PWord32(Integer(Src)+ 8)^;
              PWord32(@block[12])^ := PWord32(@(PInitVector^[12]))^ xor PWord32(Integer(Src)+12)^;
              RijndaelEncrypt(@FEncryptRK, Nr, @block, Dest);
              PInitVector := Dest;
              Inc(PWord8(Src),  SizeOf(TBlockWord8));
              Inc(PWord8(Dest), SizeOf(TBlockWord8));
            end;
...
constructor TAESChipher.Create;
begin
  Mode := cmECB;
  KeyBits := ck128;
  KeyNeedUpdate := True;
...
{$IFDEF DSRAES_TEST}
procedure TAESChipher.BlockDecryptRounds(block: PBlockWord8; rounds: integer);
begin
  RijndaelDecryptRound(@FDecryptRk, Nr, block, rounds);
end;

procedure TAESChipher.BlockEncryptRounds(block: PBlockWord8; rounds: integer);
begin
  RijndaelEncryptRound(@FEncryptRk, Nr, block, rounds);
end;
{$ENDIF}

function TAESChipher.GetKeyBitsAsInteger: integer;
begin
  Result := Ord(FKeyBits)*64 + 128;
end;

procedure TAESChipher.SetKey(const Value: String);
begin
...
procedure TAESChipher.SetKeyBits(const Value: TAESChipherKeyBits);
begin
  if FKeyBits<>Value then begin
    FKeyBits := Value;
    KeyNeedUpdate := True;
  end;
end;

procedure TAESChipher.UpdateKeyData(Forced: boolean);
var
  CipherKey: TArrayWord8;
  iKeyBits: integer;
begin
  if Forced or KeyNeedUpdate then begin
    iKeyBits := GetKeyBitsAsInteger;
    FillChar(CipherKey, SizeOf(CipherKey), 0);
    Move(PWord8(Key)^, CipherKey, Min(Length(Key)*SizeOf(Char), iKeyBits div 8));

    Nr := RijndaelKeySetupEnc(@FEncryptRK, @CipherKey, iKeyBits);
    RijndaelKeySetupDec(@FDecryptRK, @CipherKey, iKeyBits);

    KeyNeedUpdate := False;
  end;
end;

function TAESChipher.BufferDecrypt(Src: Pointer; SrcCount: integer;
  Dest: Pointer; var DestCount: integer): integer;
var
  i, NumBlocks: integer;
  PInitVector: PBlockWord8;
  block: TBlockWord8;
begin
  Result := 0;
  if (Src=nil) or (SrcCount=0) then begin
    Result := AESCIPHER_NO_DATA;
    Exit;
  end;
  if ((SrcCount-SizeOf(Integer)) mod SizeOf(TBlockWord8)) <> 0 then Exit;
  i := PInteger(Src)^;
  if i<0 then begin
    Result := AESCIPHER_BAD_DATA;
    Exit;
  end;
  if DestCount<i then begin
    DestCount := i;
    Result := AESCIPHER_NEED_MORE_DEST_SIZE;
    Exit;
  end;
  if Dest=nil then begin
    Result := AESCIPHER_NO_DEST;
  ...
  DestCount := i;
  Inc(PWord8(Src), SizeOf(Integer)); // skip buffer size

  NumBlocks := (SrcCount-SizeOf(Integer)) div SizeOf(TBlockWord8);
  case Mode of
    cmECB:
          begin
            {decrypt blocks}
            ...
            {decrypt last block}
            RijndaelDecrypt(@FDecryptRK, Nr, Src, @block);
            i := DestCount mod SizeOf(TBlockWord8); // last dest size
            if i=0 then i := SizeOf(TBlockWord8);
            Move(block, Dest^, i);
    ...
    cmCBC:
          begin
            ...
            {decrypt last block}
            RijndaelDecrypt(@FDecryptRK, Nr, Src, @block);
            PWord32(@block[ 0])^ := PWord32(@block[ 0])^ xor PWord32(@(PInitVector^[ 0]))^;
            PWord32(@block[ 4])^ := PWord32(@block[ 4])^ xor PWord32(@(PInitVector^[ 4]))^;
            PWord32(@block[ 8])^ := PWord32(@block[ 8])^ xor PWord32(@(PInitVector^[ 8]))^;
            PWord32(@block[12])^ := PWord32(@block[12])^ xor PWord32(@(PInitVector^[12]))^;
            i := DestCount mod SizeOf(TBlockWord8); // last dest size
            if i=0 then i := SizeOf(TBlockWord8);
            Move(block, Dest^, i);
...
function TAESChipher.BufferEncrypt(Src: Pointer; SrcCount: integer;
  Dest: Pointer; var DestCount: integer): integer;
var
  i, NumBlocks: integer;
  PInitVector: PBlockWord8;
...
begin
  ...
  i := SrcCount mod SizeOf(TBlockWord8);
  if i<>0 then i := 1; // increment numbers of result blocks
  i := ((SrcCount div SizeOf(TBlockWord8))+i)*SizeOf(TBlockWord8) + SizeOf(Integer);
  if DestCount<i then begin
    DestCount := i;
    Result := AESCIPHER_NEED_MORE_DEST_SIZE;
    Exit;
  ...
  UpdateKeyData(False);
  NumBlocks := SrcCount div SizeOf(TBlockWord8);
  {save source size}
  PInteger(Dest)^ := SrcCount;
  Inc(PWord8(Dest), SizeOf(Integer));
    ...
    cmCBC:
          begin
            ...
            if i <> 0 then begin
              FillChar(block, SizeOf(block), 0);
              Move(Src^, block, i);
              PWord32(@block[ 0])^ := PWord32(@(PInitVector^[ 0]))^ xor PWord32(@block[ 0])^;
              PWord32(@block[ 4])^ := PWord32(@(PInitVector^[ 4]))^ xor PWord32(@block[ 4])^;
              PWord32(@block[ 8])^ := PWord32(@(PInitVector^[ 8]))^ xor PWord32(@block[ 8])^;
              PWord32(@block[12])^ := PWord32(@(PInitVector^[12]))^ xor PWord32(@block[12])^;
              RijndaelEncrypt(@FEncryptRK, Nr, @block, Dest);
            end;
            Result := SrcCount;
          end;
...
function TAESChipher.StreamDecrypt(Src, Dest: TStream): integer;
var
  i, NumBlocks, SrcCount, DestCount: integer;
  PInitVector: PBlockWord8;
  InBlock, OutBlock, TempBlock: TBlockWord8;
begin
  Result := 0;
  if Src=nil then begin
    Result := AESCIPHER_NO_DATA;
  ...
  SrcCount := Src.Size - Src.Position;
  if SrcCount mod SizeOf(TBlockWord8) <> SizeOf(Integer) then begin
    Result := AESCIPHER_BAD_DATASIZE;
    Exit;
  end;

  UpdateKeyData(False); //fixed 08.09.2003
  Src.Read(DestCount, SizeOf(Integer));
  NumBlocks := (SrcCount-SizeOf(Integer)) div SizeOf(TBlockWord8);
  case Mode of
    cmECB:
          begin
            {decrypt blocks}
            for i:=1 to NumBlocks-1 do begin
              Src.Read(InBlock, SizeOf(InBlock));
              RijndaelDecrypt(@FDecryptRK, Nr, @InBlock, @OutBlock);
              Dest.Write(OutBlock, SizeOf(OutBlock));
            end;
            {decrypt last block}
            Src.Read(InBlock, SizeOf(InBlock));
            RijndaelDecrypt(@FDecryptRK, Nr, @InBlock, @OutBlock);
    ...
    cmCBC:
          begin
            ...
            for i:=1 to NumBlocks-1 do begin
              Src.Read(InBlock, SizeOf(InBlock));
              RijndaelDecrypt(@FDecryptRK, Nr, @InBlock, @OutBlock);
              PWord32(@OutBlock[ 0])^ := PWord32(@OutBlock[ 0])^ xor PWord32(@(PInitVector^[ 0]))^;
              PWord32(@OutBlock[ 4])^ := PWord32(@OutBlock[ 4])^ xor PWord32(@(PInitVector^[ 4]))^;
              PWord32(@OutBlock[ 8])^ := PWord32(@OutBlock[ 8])^ xor PWord32(@(PInitVector^[ 8]))^;
              PWord32(@OutBlock[12])^ := PWord32(@OutBlock[12])^ xor PWord32(@(PInitVector^[12]))^;
              Dest.Write(OutBlock, SizeOf(OutBlock));
              TempBlock := InBlock;
              PInitVector := @TempBlock;
            end;
            {decrypt last block}
            Src.Read(InBlock, SizeOf(InBlock));
            RijndaelDecrypt(@FDecryptRK, Nr, @InBlock, @OutBlock);
...
function TAESChipher.StreamEncrypt(Src, Dest: TStream): integer;
var
  i, NumBlocks, SrcCount: integer;
  PInitVector: PBlockWord8;
  InBlock, OutBlock: TBlockWord8;
begin
  Result := 0;
  if Src=nil then begin
    Result := AESCIPHER_NO_DATA;
  ...
  NumBlocks := SrcCount div SizeOf(TBlockWord8);
  {save source size}
  Dest.Write(SrcCount, SizeOf(Integer));
  ...
  case Mode of
    cmECB:
          begin
            ...
            for i:=1 to NumBlocks do begin
              Src.Read(InBlock, SizeOf(InBlock));
              RijndaelEncrypt(@FEncryptRK, Nr, @InBlock, @OutBlock);
              Dest.Write(OutBlock, SizeOf(OutBlock));
            end;
            {encrypt last block}
            i := SrcCount mod SizeOf(TBlockWord8);
    ...
    cmCBC:
          begin
            ...
            for i:=1 to NumBlocks do begin
              Src.Read(InBlock, SizeOf(InBlock));
              PWord32(@InBlock[ 0])^ := PWord32(@(PInitVector^[ 0]))^ xor PWord32(@InBlock[ 0])^;
              PWord32(@InBlock[ 4])^ := PWord32(@(PInitVector^[ 4]))^ xor PWord32(@InBlock[ 4])^;
              PWord32(@InBlock[ 8])^ := PWord32(@(PInitVector^[ 8]))^ xor PWord32(@InBlock[ 8])^;
              PWord32(@InBlock[12])^ := PWord32(@(PInitVector^[12]))^ xor PWord32(@InBlock[12])^;
              RijndaelEncrypt(@FEncryptRK, Nr, @InBlock, @OutBlock);
              PInitVector := @OutBlock;
              Dest.Write(OutBlock, SizeOf(OutBlock));
            end;
            {encrypt last block}
            i := SrcCount mod SizeOf(TBlockWord8);
...
function TAESChipher.StringDecrypt(const Value: string): string;
var
  OutSize: integer;
begin
  ...
  OutSize := 0;
  BufferDecrypt(Pointer(Value), Length(Value)*SizeOf(Char), nil, OutSize);
  SetLength(Result, OutSize div SizeOf(Char));
  BufferDecrypt(Pointer(Value), Length(Value)*SizeOf(Char), Pointer(Result), OutSize);
end;

function TAESChipher.StringEncrypt(const Value: string): string;
var
  OutSize: integer;
begin
  if Length(Value)=0 then begin
  ...
  OutSize := 0;
  BufferEncrypt(Pointer(Value), Length(Value)*SizeOf(Char), nil, OutSize);
  SetLength(Result, OutSize div SizeOf(Char));
  BufferEncrypt(Pointer(Value), Length(Value)*SizeOf(Char), Pointer(Result), OutSize);
end;
