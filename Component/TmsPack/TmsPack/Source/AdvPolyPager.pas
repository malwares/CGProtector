...
uses
  ...
  Dialogs, Forms, ExtCtrls, GDIPFill, AdvStyleIF, CustomItemsContainer,
  AdvGDIP, AdvVerticalPolyList, GDIPBase, GDIPCustomItem, GDIPPictureContainer, ImgList
{$IFNDEF TMSDOTNET}
  , AxCtrls
{$ENDIF};
...
begin
  ...
  begin
    ...
    repeat
      p := p.Parent;
    until (p is TForm){$IFNDEF TMSDOTNET} or (p is TActiveForm)
    {$ENDIF} or not Assigned(p);
...
var
  ...
  p: integer;
begin
  if (Item = FSelItem) and (PageFromItem(Item) <> -1) then
  begin
    Allow := false;
    Exit;
  end;
