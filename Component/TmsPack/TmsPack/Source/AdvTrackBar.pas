...
begin
  ...
  begin
    ...
    if (FMax <> FMin) then
      i := ((R.Right - R.Left) - GetThumbWidth) * (FPosition - FMin) div (Fmax - FMin)
    else
      i := 0;
  ...
  begin
    ...
    if (FMax <> FMin) then
      i := ((R.Bottom - R.Top) - GetThumbWidth) * (FPosition - FMin) div (Fmax - FMin)
    else
      i := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not Assigned(FOnDrawTick) and (P.X <= OldX + 1) then
        Continue;
      OldX := P.X;
...
begin
  ...
  if not IsRating and not PtInRect(ThumbRect, Point(X,Y)) and (Button = mbLeft) and
    (MoveOnClick = mcMove) and not (inminbtn or inmaxbtn) then
  begin
    R := TrackClientRect;
    if (Orientation = trHorizontal) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Orientation = trHorizontal) then
      begin
        W := (R.Right - R.Left) - (ThumbRight.Width div 2);
        P := Round((((X - R.Left) - (ThumbRight.Width div 2)) / W) * (FMax - FMin) + FMin);
      end
      else // Orientation = trVertical
      begin
        H := (R.Bottom - R.Top) - (ThumbRight.Width div 2);
        P := Round(((H - Y + R.Top) / H) * (FMax - FMin) + FMin);
      end;
