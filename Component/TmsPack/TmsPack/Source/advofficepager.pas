...
procedure DrawVistaGradient(Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC: TColor;
   ...
   AntiAlias: TAntiAlias; RoundEdges: Boolean; RotateLeftRight: Boolean; Progress: TTabProgress; Direction: TTabPosition = tpTop); overload;
begin
...
begin
    ...
    begin
      ...
      begin
        ...
        DrawVistaGradient(FBkgCache.Canvas, Rect(0, 0, FBkgCache.Width, FBkgCache.Height), Color, ColorTo, ColorMirror, ColorMirrorTo, BorderColor,
          Gradient, GradientMirror, '', Font, Layout, Enabled, False, FAdvOfficePager.AntiAlias, FAdvOfficePager.FCurrentOfficePagerStyler.RoundEdges, True, nil, FAdvOfficePager.TabPosition);
        Canvas.Draw(R.Left, R.Top, FBkgCache);
        FValidCache := true;
      end
      else
  ...
  begin
    ...
    if not FAdvOfficePager.IsGlass and FAdvOfficePager.FCurrentOfficePagerStyler.RoundEdges and (FAdvOfficePager.PageMargin > 0) then
    begin
      // Clean up edges
...
begin
  ...
  {  if Enabled or (DisabledImages = nil) then
    ...
    if not (csDesigning in ComponentState) and Glow and Assigned(FBrGlowTimer) and AdvPages[PageIndex].Glow and (AdvPages[PageIndex].GlowColor <> clNone) then
    begin
      ...
      begin
        ...
        if UseOldDrawing then
          DrawVistaGradient(bmp.Canvas, Rect(0, 0, R.Right - R.Left, R.Bottom-R.Top),GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo, PenColor,
            GradU, GradB, '', {Canvas.}Font, Layout, Enabled, False, FAntiAlias, True{FCurrentToolBarStyler.RoundEdges}, RotateLR, AdvPages[PageIndex].Progress, TabPos)
        else
        begin
          case TabPosition of
      ...
      begin
        case TabPosition of
          ...
          begin
            ...
            //--- Draw 3D effect
            if not AdvPages[PageIndex].Progress.Visible then
            begin
            ...
            if (HighLightClr <> clNone) and not (AdvPages[PageIndex].Progress.Visible and ShowNonSelectedTabs and (PageIndex <> ActivePageIndex)) then
            begin
...
begin
  ...
  begin
    ...
    begin
      ...
      {  OnShow Should have already been called in showing changed.
      if not (csLoading in ComponentState) then
...
begin
  ...
  begin
    ...
    begin
      ...
      gpPen := TGPPen.Create(ColorToARGB(FCurrentOfficePagerStyler.PageAppearance.BorderColor),1);
...
begin
  ...
  if ShowTabHint then
  begin
    //outputdebugstring(pchar('cmhintshow:'+inttostr(FHintPageIndex)));
...
begin
  if not (csDesigning in ComponentState) and Glow and IsAnyTabGlowing then
  begin
    CreateGlowTimer;
