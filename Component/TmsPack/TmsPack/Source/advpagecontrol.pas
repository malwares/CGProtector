...
uses
  ...
  Graphics, ImgList, ComCtrls, Buttons, Menus
{$IFDEF DELPHI7_LVL}
  , Types
{$ENDIF}
{$IFNDEF TMSDOTNET}
...
begin
  ...
  if Assigned(OnCanChange) then
    OnCanChange(Self, ActivePageIndex, Page.TabIndex, AllowChange);

  if (Page <> nil) and (Page <> ActivePage) and AllowChange then
  begin
    ActivePageIndex := Page.TabIndex;
    Change;
  end;
end;
