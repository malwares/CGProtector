...
type
  ...
  published
    ...
    property AutoOpenMenus: Boolean read FAutoOpenMenus write SetAutoOpenMenus default false;
...
begin
  ...
  begin
    for I := 0 to MenuItems.Count - 1 do
    begin
      if IsAccel(CharCode, MenuItems[I].Caption) and CanFocus and TabStop and not Assigned(MenuItems[I].frm) then
      begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            while (SelectedItem.item.ItemType <> itNormal) do
            begin
              DoItemSelection(Key, SelectedItem, Sec, Secit);
              newitem.section := Sections[sec];
              newitem.item := Sections[sec].Items[secit];
...
begin
  ...
  if Assigned(Menu) and Assigned(MenuItem) then
  begin
    if not FFloatingEvent and MenuItem.IsFloating then
    begin
      FFloatingEvent := true;
      menu.FMenuAutoOpen := false;
